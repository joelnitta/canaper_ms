...
2. Here, we describe `canaper`, a new R package that provides functions to conduct CANAPE in R.
...
This became possible with the development of phylogenetic measures of biodiversity, such as phylogenetic diversity [PD\; @Faith1992] and phylogenetic endemism [PE\; @Faith1992; @Rosauer2009] (Box 1).
Such analyses are becoming much more common due to the widespread availability of robust molecular phylogenies [e.g., @Mishler2020; @Nitta2022a; @Thornhill2017].
...
R is well-suited for reproducible workflows and is extremely versatile thanks to the thousands of community-contributed packages that extend its utility.
...
Also, while Biodiverse conducted the metrics needed to categorize endemism type, it did not actually perform this categorization.
...
Here, we describe a new R package that implements CANAPE completely in R, `canaper`.
We strived to make `canaper` simple to use and efficient.
Parallel computing can be enabled with a single line of code.
Randomization algorithms are provided via the `vegan` package [@Oksanen2017], which comes with \> 30 pre-defined algorithms as well as the option to provide a custom, user-defined algorithm.
...
However, when PD and PE are calculated only presence-absence information is used (i.e., abundance weighting is not used), so identical results will be obtained whether the input data is abundance or abundance that has been converted to presence-absence.
...
Next, an alternative phylogeny (also referred to as the "comparison tree") is constructed that has non-zero branch lengths each set to an equal value, then rescaled to a total length of 1.
...
In CANAPE, areas (grid-cells) with extremely high or low RPE are designated as paleo- or neo-endemic, respectively.
...
These algorithms preserve the number of sites occupied by each species and the richness of each site, and both produce results comparable to the randomization algorithm in Biodiverse, `rand_structured` (see "Example: Australian *Acacia*").
...
Summary statistics include the mean and standard deviation of PD, RPD, PE, and RPE of the random communities and comparisons of observed values to the random communities including standard effect size and rank, which is used to calculate *p*-value.
...
This way, for any input matrix of size $N \times M$ and $n$ random replicates, where $N$ is number of rows (sites) and $M$ is number columns (species), $n$ summary matrices each with $N$ rows and one column per summary statistic are stored, rather than $n$ matrices of size $N \times M$.
...
In order to be considered significantly endemic, a given grid-cell must first have significantly high `pe_obs` or `pe_alt` or both.
...
Joel H. Nitta conceived the ideas and designed methodology; Joel H. Nitta and Shawn W. Laffan analysed the data; Joel H. Nitta led the writing of the manuscript.
...
tibble(
  dataset = names(comms),
  n_species_comm = map_dbl(comms, ncol),
  n_sites = map_dbl(comms, nrow),
  n_species_tree = map_dbl(phylos, ape::Ntip)
) %>%
  knitr::kable()
```
