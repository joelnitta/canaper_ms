...
However, when parallel computing was enabled for `canaper` with two cores, compute time dropped to `r times$acacia_canape_cpr_2$minutes` min (further increases in number of cores did not significantly decrease time).
...
`PhyloMeasures` and `EcoSimR` have both been removed from CRAN and may not be currently maintained.
