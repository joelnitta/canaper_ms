...
Joel H. Nitta^1,\*^, Shawn W. Laffan^2^, Brent D. Mishler^3^, and Wataru Iwasaki^1,4,5,6,7^
...
^3^School of Biological, Earth and Environmental Sciences, The University of New South Wales, Kensington, Australia
...
^6^Institute for Quantitative Biosciences, The University of Tokyo, Tokyo, Japan

^7^Collaborative Research Institute for Innovative Microbiology, The University of Tokyo, Tokyo, Japan
...
3. Our case study shows that `canaper` produces equivalent results to Biodiverse and can complete computations on moderately sized datasets quickly (< 15 min to reproduce a canonical study).
...
CANAPE uses phylogenetic methods to give insight into evolutionary processes that produce endemism.
In theory, centers of endemism (areas with many range-limited taxa; or in the phylogenetic sense, concentrations of range-limited branches of the phylogeny) may arise via multiple processes.
For example, previously widespread lineages may undergo extinction in all but a portion of their range (paleo-endemism), or recently diverged lineages may only occur in a small area (neo-endemism). 
...
CANAPE involves analyzing observed patterns of PE in comparison with a null model to infer first whether an area is significantly high in PE, then classifying those areas into centers of paleo-endemism, neo-endemism, or mixed endemism.
...
Also, while Biodiverse conducted the metrics needed to categorize endemism type, it did not actually perform this categorization (although this feature is planned to be added to a future version of Biodiverse).
Furthermore, the Biodiverse GUI alone does not support parallel processing, which is needed for large datasets.
Parallel processing and automation of Biodiverse analyses is only possible using perl scripts.
...
We strived to make `canaper` simple to use and efficient.
Parallel computing can be enabled with a single line of code.
`canaper` has undergone thorough code review through ROpenSci [@Boettiger2015] and is verified against a large number of unit tests (>95% coverage).
...
The first argument, `comm` is a community dataframe (or matrix). 
...
Either type of algorithm will accept either type of data, but binary algorithms will convert abundance data to binary and return a binary matrix (dataframe).
...
Generally, larger matrices with more skew (overabundance of zeros) will require more iterations (ADD CITATION).
...
The *Acacia* dataset is large (@tbl-datasets) and highly skewed (`r acacia_non_empty` zeros).
...
Both `cpr_classify_endem()` and `cpr_classify_signif()` take a dataframe as their first argument and return a dataframe as output, so they are "pipe friendly", i.e., can be chained together using the pipe (`%>%` or `|>`).
...
Each is a list with two named elements, `comm` (community matrix, dataframe) and `phy` (phylogenetic tree, list of class `"phylo"`).
...
Conducting CANAPE on Australian *Acacia* using `canaper` is simple, requiring only two commands:
...
a <- ggplot(acacia_canape, aes(x = long, y = lat, fill = endem_type)) +
  ...
  # (for CVD-safe colors see cpr_endem_cols or cpr_endem_cols_2)
...
Alternate color palettes (e.g., `cpr_endem_cols`) are also vailable that include CVD-safe colors [@Okabe2002].
...
# Tally number of significant cells in canper vs. Biodiverse
sig_tally <-
  comp_table %>%
  mutate(
...
The total number of significant cells was nearly identical between the two (*n* = `r sig_tally$canaper$significant$n_sites` and `r sig_tally$biodiverse$significant$n_sites` sites, `canaper` and Biodiverse, respectively).
...
# Load times
tar_load(acacia_biod_elapsed_min)

# Enforce dependency on acacia_canape_cpr_1
tar_load(acacia_canape_cpr_1,
  store = here::here("_targets"))
...
This research supported in part by Japan Society for the Promotion of Science (Kakenhi) Grant numbers 16H06279 and 22H04925.
...
Joel H. Nitta conceived the ideas and designed methodology; Joel H. Nitta and Shawn W. Laffan analyzed the data; Joel H. Nitta led the writing of the manuscript.
All authors contributed critically to the drafts and gave final approval for publication.

# Data availability

All data and code are available in the `canaper` R package, which is distributed by the Comprehensive R Archive Network (CRAN).
Source code is available on GitHub (<https://github.com/ropensci/canaper>) and Zenodo (<https://doi.org/10.5281/zenodo.5094032>).
...
tibble(
  ...
  `*n* species (tree)` = map_dbl(phylos, ape::Ntip)
) %>%
  knitr::kable()
```
...
- Mixed endemic: An area with a mixture of short and long, range-restricted branchlengths; may be due to multiple processes.
...
ggsave(plot = fig, file = "ms/figure_1.pdf", width = 150, height = 150, units = "mm")
...
ggsave(plot = fig, file = "ms/figure_2.pdf", width = 100, height = 200, units = "mm")
