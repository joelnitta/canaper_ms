...
^2^Earth and Sustainability Science Research Centre, School of Biological, Earth and Environmental Sciences, The University of New South Wales, Sydney, NSW, Australia

^3^Jepson and University Herbaria, and Department of Integrative Biology, University of California Berkeley, Berkeley, USA
...
Metrics of phylogenetic diversity including CANAPE can be measured at any taxonomic level, but for simplicity we use ’species’ here.
...
As there is no single "correct" algorithm, we have opted to provide the user with a wide range of options by supporting randomization algorithms included in the `vegan` package.
...
The following code demonstrates usage of `cpr_iter_sim()` with the *Acacia* dataset [@Mishler2014] that comes with `canaper`.
...
`phylocom` and `biod_example` are synthetic datasets compiled for testing software, and were originally published in Phylocom [@Webb2008] and Biodiverse [@Laffan2010], respectively.
...
All `canaper` analyses were run with `canaper` v`r packageVersion("canaper")` in R v4.2.1.
...
`canaper` has already been used at least five studies while in development [@Ellepola2022; @Lu2022; @Nitta2022a; @Naranjo2023; @vanGalen2023].
...
Luis Osorio and Klaus Schliep reviewed the code as part of submission to rOpenSci. Three anonymous reviewers provided comments that improved the manuscript.
...
To cite canaper or acknowledge its use, cite this Software note as follows, substituting the version of the application that you used for 'version 0':

<!-- FIXME: update journal details when available -->
Nitta, J. H. et al. 2023. canaper: Categorical analysis of neo- and paleo-endemism in R. Ecography XX: XXX--XXX (version 0)
...
Latitude and longitude projected into equal area Australian Albers (EPSG:3577) coordinate system [@Butler2007].
(b) Scatterplot comparing phylogenetic endemism (PE) of each community as measured on the original tree vs. a comparison tree with all non-zero branch lengths set to equal length, colored according to endemism type. This figure reproduces Figure 3 of @Mishler2014.
